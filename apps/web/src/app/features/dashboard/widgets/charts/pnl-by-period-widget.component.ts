...
@Component({
  selector: 'app-pnl-by-period-widget',
  standalone: true,
  imports: [CommonModule, ChartModule],
  template: `
    <div class="p-4 h-full flex flex-col">
      <div>
        <h2 class="text-xl font-black tracking-tight m-0">P&amp;L por periodo</h2>
        <p class="text-xs text-placeholder mt-1">Beneficio o pérdida agregada por {{ periodLabel }}</p>
      </div>
      <div class="flex-1 min-h-0 relative mt-4">
        @if (dataService.selectedTimeframe() !== 'all' && dataService.pnlBarChartData(); as data) {
          <p-chart type="bar" [data]="data" [options]="dataService.pnlBarChartOptions" height="100%"></p-chart>
        } @else {
...
export class PnlByPeriodWidgetComponent {
