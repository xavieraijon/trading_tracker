...
export class DashboardComponent {
  ...
  drawdownChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        displayColors: false,
        callbacks: {
          label: (item: { raw: number }) => `Drawdown: $${Math.abs(Number(item.raw)).toFixed(2)}`
        }
      }
    },
    scales: {
  ...
  onTimeframeSelect(value: 'all' | 'day' | 'week' | 'month' | 'year') {
  ...
  prepareDrawdownChartData() {
    ...
    this.drawdownChartData.set({
      labels: dataPoints.map((p: { date: string }, i: number) => {
        const date = new Date(p.date);
        if (timeframe === 'all') return date.toLocaleDateString();
        if (timeframe === 'day') return date.toLocaleDateString(undefined, { day: '2-digit', month: 'short' });
        if (timeframe === 'week') return `S${this.getWeekNumber(date)}`;
  ...
  prepareLongShortDonut(curve: { side?: string }[] | null) {
    if (!curve?.length) {
      this.longShortDonutData.set(null);
      return;
    }
  ...
  prepareWinsLossesDonut(curve: { pnl?: number }[] | null) {
    if (!curve?.length) {
      this.winsLossesDonutData.set(null);
      return;
    }
  ...
  prepareGrossPnLDonut(curve: { pnl?: number }[] | null) {
    if (!curve?.length) {
      this.grossPnLDonutData.set(null);
      return;
    }
  ...
  preparePnlByInstrumentBar(curve: { instrument?: string; pnl?: number }[] | null) {
    if (!curve?.length) {
      this.pnlByInstrumentBarData.set(null);
      return;
    }
    ...
    const sorted = Object.entries(byInstrument)
  ...
  prepareTradesByPeriodBar(curve: { date: string }[] | null) {
    const timeframe = this.selectedTimeframe();
    if (!curve?.length || timeframe === 'all') {
      this.tradesByPeriodBarData.set(null);
      return;
    }
    ...
    const labels = sorted.map(([key]) => {
      const firstInPeriod = curve.find(x => {
